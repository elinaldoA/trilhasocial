<x-app-layout>
    <x-slot name="header">
        Mensagens - Conversa com {{ $selectedUser->name }}
    </x-slot>
        ...
        <div class="w-1/3 border-r border-gray-200 dark:border-gray-700 overflow-y-auto">
            ...
            @foreach ($users as $user)
                <a href="{{ route('messages.show', $user->id) }}"
                   class="block px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-700 cursor-pointer
                   {{ $selectedUser->id === $user->id ? 'bg-indigo-600 text-white' : 'text-gray-900 dark:text-gray-100' }}">
                    {{ $user->name }}
                </a>
            @endforeach
        </div>
